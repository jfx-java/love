    ...
    $.fn.uploader = function (options, testMode) {
        return this.each(function (index) {
            options = $.extend({
                ...
                ajaxUrl: '/accepter/upRequestProject',
                testMode: false
            }, options);
            ...
            function uploadSubmitHandler () {
                if (state.fileBatch.length !== 0) {
                    if (state.fileBatch.length>5){
                        alert("最多上传五张证明图片！请重新选择！(单击红色横杠可删除对应图片)");
                        return;
                    }
                    ...
                    var topic=$("#topic").val();
                    var accepterId=$("#accepter_id").val();

                    var message=$("#message").val();
                    var telephone=$("#telephone").val();
                    var money=$("#money").val();
                    var alipyNum=$("#alipyNum").val();
                    ...
                    $("input[class='goodsNum']").each(function () {
                        goodsNum.push($(this).val());
                    });

                    var goods="";
                    for(var n=0;n<goodsName.length;n++){
                        goods+=goodsName[n]+":"+goodsNum[n]+";";
                    };

                    var certifierName=new Array();
                    $("input[name='certifier']").each(function(){
                        certifierName.push($(this).val());
                    });

                    var certifierTelephone=new Array();
                    $("input[class='certifierTelephone']").each(function () {

                        certifierTelephone.push($(this).val());
                    });

                    var certifier="";

                    for(var mm=0;mm<certifierName.length;mm++){

                        certifier+=certifierName[mm]+":"+certifierTelephone[mm]+";";
                    };

                    data.append("img",$("#file")[0].files[0]);
                    data.append("topic",topic);
                    data.append("accepterId",accepterId);
                    data.append("message",message);
                    data.append("telephone",telephone);
                    data.append("money",money);
                    data.append("alipayNum",alipyNum);
                    data.append("address",address);
                    data.append("goods",goods);
                    data.append("volunteer",volunteer);
                    data.append("endTime",endTime);
                    data.append("certifier",certifier);
